...
public class CanvasControl : MonoBehaviour
{
    ...
    [SerializeField] private GameObject panelChooseMap;//맵 고르는 화면
    [SerializeField] private GameObject panelPlay;//플레이 화면
    [SerializeField] private GameObject panelGoal;//클리어 시 화면
    [SerializeField] private GameObject panelController;//안드로이드 시 입력 UI패널
    [SerializeField] private MapButton mapButtonPrefab;//맵 버튼 프레팝 연결
    [SerializeField] private RectTransform contentTransform;//스크롤 뷰 에서 버튼 프레팝이 들어갈 위치
    [SerializeField] private Slider sliderZoom;//화면 줌 슬라이더.
    [SerializeField] private Sprite[] audioOnOff;//효과음 켜고 끄기 스프라이트 배열.
    private MapButton[] mapButton = null;//맵 버튼 배열
    
    private void Awake()//버튼이 미리 만들어져 있어야 클리어 여부가 제대로 표시된다. 그래서 Start가 아닌 Awake
    {
        //게임매니저에 있는 최대 맵 갯수만큼 배열을 만든다.
        mapButton = new MapButton[GameManager.MAX_MAP_COUNT];
        for (int i = 0; i < GameManager.MAX_MAP_COUNT; i++)
        {
            //스크롤 뷰에 맵 버튼 프레팝으로 클론을 만들어서 넣는다
