...
public class GameManager : MonoBehaviour
{
    ...
    public static string KEY_PP = "OPEN_MAP";//맵 오픈 정보를 저장 할 키.
    public static string KEY_AUDIO = "AUDIO";//효과음 상태 저장할 키.
    public static int MAX_MAP_COUNT = 96;
    public static float MAX_ZOOM = 4f;
    public static float MIN_ZOOM = 11f;
    public static float START_ZOOM = 5f;
    ...
    [SerializeField] private GameObject mapPrefab;//맵 오브텍드 프레팝
    [SerializeField] private GameObject ground;//바닥 오브젝트들 부모
    [SerializeField] private GameObject walls;//벽 오브젝트 부모
    [SerializeField] private GameObject goals;//골 오브잭트 부모
    ...
    private int? undoBoxIndex;//되돌리기 할 상자 번호.(없을 수도 있으니 null값을 가질 수 있게)nullable?
    private bool isRunGoalCoroutine;//코루틴 하나만 돌게 체크하는 변수.
    private float targetZoom;//줌인아운 크기 변수
    private Camera mainCamera;//카메라
    private AudioSource audioSource;//효과음 소스
    private AudioState audioEnable;//효과음의 상태 저장할 변수.
    ...
    private void Awake()
    {
        ...
        mainCamera.orthographicSize = START_ZOOM;
    ...
    private void Start()
    {
        //KEY_PP값이 없거나 이전 값이면 값을 만들어 저장한다.
    ...
    private void ClearAllObject()
    {
        ...
        DeleteChild(boxes);
        DeleteChild(goals);
        //맵 고르기 화면에서는 플레이어 안 보이게
        player.SetActive(false);
        //되돌리기 상자는 선택 된 것이 없으니 null.
        undoBoxIndex = null;
        //코루틴이 돌고 있다면 모두 멈춤
    ...
    private void MoveProcess(float moveX, float moveY)
    {
        ...
        //바닥이면 (board에는 ground아니면 wall만 남아있다)
        if (board[(int)tryPlayerPos.x, (int)tryPlayerPos.y] == MapEnum.Ground)
        {
            //움직임이 발생했으므로 버튼이 다시하기, 되돌리기 버튼 활성화
            ...
            else//움직일 위치에 상자가 없다면
            {
                //되돌리기 위치에 플레이어 위치를 넣고
                undoPosition = playerPosition;
                //되돌리기 상자는 없으므로 null. nullable이라서 int에도 null쓸 수 있음.
                undoBoxIndex = null;
                //플레이어 이동
                playerPosition = tryPlayerPos;
                //이동 효과음
...
private void Update()
    {
        //카메라 줌 인 아웃 으로 맵 전체를 한 화면에 볼 수 있다.

        //클리어 이후에도 줌 아웃은 되게 한다.

#if UNITY_EDITOR || UNITY_STANDALONE//안드로이드 실 기기에서는 화면상의 슬라이더를 이용한다.
//프로퍼티를 이용해 min, max 값사이로 조정한다.
        TargetZoom -= Input.mouseScrollDelta.y * SENSITIVITY;
#endif
        mainCamera.orthographicSize = Mathf.MoveTowards(mainCamera.orthographicSize, targetZoom, ZOOM_SPEED * Time.deltaTime);
    ...
    private IEnumerator Congratulations()
    {
        ...
        //클리어 정보 현재 맵 위치는 'C'로 바꾼다
        SetOpenClearMapData(mapNumber, "C");//stirng이기에 ""사용
        //다음 맵은 이미 오픈되어 있거나 클리어된 상태이면 아무것도 하지 않는다.
        SetOpenClearMapData(mapNumber+1,"O",'X');//두번째 것은 char형이기에 ''사용.
        //맵 선택 화면에서도 클리어 했다 표시, 다시하기 되돌리기 버튼은 비활성화
        canvas.SetMapCleared(mapNumber);
        //1초 기다렸다가(상자가 움직일 시간이 필요)
        yield return new WaitForSeconds(1f);//1초 기다림.
        //정면을 바라보는 애니메이션을 한번 실행한다.
    ...
    //JSON데이터로부터 맵 데이터 만들기
    private void MakeMapDateFromJSON()
    {
        ...
        for(int y = 0; y < map.SizeY; y++)
        {
            for(int x = 0; x<map.SizeX;x++)
            {
                ...
                index++;//인덱스 + 1 로 다음 char탐색.
            }
            index++;//줄바꿈char가 있으므로 index + 1.
        }
    }
    //화면에 오브젝트 생성
    private void MakeObject(MapEnum index, int x, int y)
    {
        ...
        if (index == MapEnum.Goal)
        {
            target = goals.transform;
        }
        else if (index == MapEnum.Wall)
        {
            target = walls.transform;
        }
        else if (index == MapEnum.Box)
        {
            target = boxes.transform;
        }

        //프레팝으로 오브젝트 생성 후, 타입을 지정한다. 한 줄로 간단하게? 만듬
        Instantiate(mapPrefab, GetPosition(x, y), quaternion.identity, target).GetComponent<StuffChange>().MapEnumType = index;
    }
    //맵 정보 초기화
    private void InitializeMap()
    {
        ...
        for(int y = 0; y < map.SizeY; y++)
        {
            for (int x = 0; x < map.SizeX; x++)
            {
                ...
                //벽 만들고
                if (board[x, y] == MapEnum.Wall)
                {
                    MakeObject(MapEnum.Wall, x, y);
                }

                //골 리스트에 좌표가 있으면 골 만들고
                if (goalList.Contains(new Vector2(x, y)))
                {
                    MakeObject(MapEnum.Goal, x, y);
                }

                //상자 리스트에 좌표가 있으면 상자 만든다
                if (boxList.Contains(new Vector2(x, y)))
                {
                    MakeObject(MapEnum.Box, x, y);
                }
            }
        }
    ...
    public void Undo()
    {
        ...
        {
            //되돌릴 상자는 현재 플레이어의 위치(플레이어가 상자를 밀어서 그 자리에 갔으므로)
            boxList[(int)undoBoxIndex] = playerPosition;//undoBoxIndex가 nullable형이므로 int형으로 형 변환을 시킨다.
        }
        //플레이어의 위치는 되돌리기 위치에 저장해 좋은 위치로
        playerPosition = undoPosition;
        //되돌리기 버튼은 한 번 사용했으므로 안 눌리게 만듬.
        canvas.SetUndoButton(false);
    }
    ...
    //치트 : 맵 오픈.
    [ContextMenu("Sokoban/Cheat : Open all map (in GameManager)", false, 3)]
    private void CheatOpenOneMap()
    {
        SetOpenClearMapData(mapNumber, "O");
        Debug.Log($"Cheat Activate! mao{mapNumber:00} Opened!");
    }
    ...
    //JSON데이터를 맵 데이터로 읽기
    [ContextMenu("Sokoban/Load JSON map from JSON", false, 5)]
